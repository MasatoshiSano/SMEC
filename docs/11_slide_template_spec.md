...
|---|---|
| 設問文 | 過去問の設問文をそのまま引用 |
| 選択肢 | ア〜オをそのまま列挙し、正解の選択肢だけ罫線と淡い背景で強調する |
| 解説 | 正解の理由＋主要な誤答の理由を1段落に凝縮（全選択肢を律儀に解説すると長すぎるため、要点のみ） |
| 出典 | 過去問PDFのパス・年度・設問番号、正解PDFのパスを`Space Mono`の小さい文字で明記 |

```css
.examq{flex:1;display:flex;flex-direction:column;min-height:0;overflow:hidden;gap:0.55cqw;}
...
`<details>`によるクリックで解答を隠す方式は採用しない。カードは16:9固定で高さが伸びないため、折りたたんだ内容を開いたときに`overflow:hidden`で見えなくなってしまう（`problem_sets/1st_stage/exercises/`のMarkdown版は`<details>`で正しく機能するが、これは可変高さのページだからであり、固定高さのスライドカードでは同じ手法は使えない）。スライドは「参照用に答えを見せておく」形にする。
