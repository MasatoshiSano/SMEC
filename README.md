...
problem_sets/
  ...
  1st_stage/exercises/         論点別演習（設問・解答・解説）。財務会計・企業経営理論で試作中
```
...
`problem_sets/README.md` に方針を記載。市販の「過去問完全マスター」方式（論点別×頻出度A/B/Cランク）を採用し、フェーズ1（論点見出しの作成）・フェーズ2（過去問10年分を分析して頻出ランクを確定）ともに7科目で完了。フェーズ3（頻出論点順の演習化）は財務・会計（B）・企業経営理論（C）の2科目・各6問で試作中（`problem_sets/1st_stage/exercises/`）。これに合わせた学習スケジュールを `docs/10_study_schedule.md` にまとめた。
...
- `problem_sets/1st_stage/` はフェーズ1・2完了（論点見出し・頻出ランク確定）。フェーズ3（頻出論点順の解答解説付き演習化）は財務・会計・企業経営理論の2科目・各6問で試作中。フォーマットが確定次第、残り5科目・他年度に展開する（`docs/10_study_schedule.md` のフェーズBで並行実施予定）。
