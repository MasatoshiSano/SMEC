...
教科書は文章中心のため、スライド教材への展開を検討中。デザインの方向性（配色・構成ルール）は `docs/11_slide_template_spec.md` に確定済みで、`docs/slide_template/` にサンプル一式（図解主体：汎用パターン12種＋科目別専用グラフ21種／文章主体：T1〜T3の3種／全パターン共通の「頻出データ帯」）がある。論点の性質に応じて図解主体・文章主体を使い分け、全論点共通で頻出ランク・出題年度の実データ帯をカード下部に付ける方針。教科書全文を流し込んだ検証（C-7・C-8）は完了しているが、全226論点を通しで作る本番作業はこれから。
